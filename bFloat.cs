...
namespace biosl5
{
    class bFloat
    {
                    ...
                    if (!isDirect)
                    {
                        ...
                        while (!isDone)
                        {
                            ...
                            try //if the user did not enter an value, then its no good, return a random value.
                            {
                                ...
                                x = float.Parse(_terms[0]);
                                _operator = _terms[1]; //*, /, +, -      (mathOperators)
                                y = float.Parse(_terms[2]);
        ...
        static float doExpression(string operation) //example : "2 * 3"
        {
            ...
            try
            {
                float x = float.Parse(words[0]); //must be signed
                float y = float.Parse(words[2]); //must be signed
        ...
        static float getValue(string newVal)
        {
            try
            {
